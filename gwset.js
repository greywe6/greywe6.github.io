(function () {
    ...
    if(back == 'delete') Lampa.Activity.active().activity.render().find('.view--modss_online').remove();
		  ...
		  if (!back && Lampa.Storage.field('mods_onl')) {
		    setTimeout(function (){
		      if(Lampa.Controller.enabled().name == 'full_start' && !Lampa.Activity.active().activity.render().find('.view--online').length) {
        ...
        btn.unbind('hover:enter hover.click').on('hover:enter hover.click', function () {
  		    inf();
          Lampa.Activity.active().activity.render().find('.view--online').html(Lampa.Lang.translate(ico + '<span>'+_this.title+'</span>'));
