...
(function ( backendhost, backendver ) {
    ...
    function Filmix(component, _object) {
      ...
      this.search = function (_object, title, similar) {
        ...
        if (!window.filmix.is_max_qualitie && token.indexOf('aaaabbbbccccddddeeeeffff') == -1) {
          ...
          return;
        } if (window.filmix.enable && !window.filmix.is_max_qualitie) {
          window.filmix.is_max_qualitie = true;
          network.clear(); network.timeout(15000);
          network["native"]( backendhost+'/lampa/filmixurl?'+backendver + '&id=' + object.movie.id + '&title=' + encodeURI(object.search) , function(found) {
            if (found && found.hash) {
              window.filmix.replace = true; window.filmix.hash = found.hash; window.filmix.max_qualitie = 2160;
            }
            component.search(object, title);
          }, function (a, c) {
            component.empty(network.errorDecode(a, c));
    ...
    function startPlugin() {
      window.plugin_FilmixPVA = { ready: true, mini: false };
    ...
    function startSources(destroy) {
      ...
      if (Lampa.Storage.get('pva_sources', false)) { 
        var ScriptItem = 'https://greywe6.github.io/gw_sr.js';
